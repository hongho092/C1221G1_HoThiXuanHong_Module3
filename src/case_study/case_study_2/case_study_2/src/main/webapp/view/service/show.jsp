...
<div class="col-12 border border-1 bg-light text-center">
    <h4 class="mt-5">Danh sách DỊCH VỤ</h4><br>
    <button class="btn btn-info mt-3"><a href="/service?action=create" class="text-decoration-none">Tạo dịch vụ</a></button><br>
    ...
    <table class="table table-striped text-center" id="tableStudent">
        ...
        <tbody>
        <h:forEach items="${services}" var="sv" varStatus="hong">
            <tr>
                <td>${hong.count}</td>
                <td>${sv.maDichVu}</td>
                <td>${sv.tenDichVu}</td>
                <td>${sv.dienTich}</td>
                <td>${sv.chiPhiThue}</td>
                <td>${sv.soNguoiToiDa}</td>
                <td>
                    <h:forEach items="${kt.entrySet()}" var="kt">
                        <h:if test="${sv.maKieuThue == kt.getKey()}">
                            ${kt.getValue()}
                        </h:if>
                    </h:forEach>
                </td>
                <td>
                    <h:forEach items="${ldv.entrySet()}" var="ldv">
                        <h:if test="${sv.maLoaiDichVu == ldv.getKey()}">
                            ${ldv.getValue()}
                        </h:if>
                    </h:forEach>
                </td>
                <td>${sv.tieuChuanPhong}</td>
                <td>${sv.moTaTienNghiKhac}</td>
                <td>${sv.dienTichHoBoi}</td>
                <td>${sv.soTang}</td>
            </tr>
        </h:forEach>
        </tbody>
